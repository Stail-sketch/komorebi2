...
**前提**: `night2_clear`
**操作**: チャット（キツネ団長）で **クイズ5問** に正解する

### チャットフロー
1. ウォームアップ会話（3段階の選択肢会話）
2. 「サイト読んだ？」→「まだ」なら会話終了（ステージ進まず）、「読んだ」ならクイズへ
3. クイズ5問（難易度上昇）→ 全問正解でNight3解放

### クイズ問題（番組調査レポートから出題）

| # | 問題 | 答え | 出典 |
|---|------|------|------|
| Q1 | 番組で一番視聴率が高かった回って何%？ | `9.1` | report_ratings.html（1993年度最高値） |
| Q2 | 視聴率0%が記録された日は何月何日？ | `11月13日` | report_ratings.html（特異な視聴データ） |
| Q3 | 1996年改修の技術者は何日間泊まり込み？ | `3日` | report_recording.html（1996年改修セクション） |
| Q4 | よわりが見つけたVHSのタイムスタンプは？ | `2時34分` | report_recording.html（夜間収録セクション） |
| Q5 | 制作会社の社長がよく使ってた言葉は？ | `理想の子ども` | testimony.html（関係者証言） |

**チャット解放**: Night2クリア後のメールで添付ファイルをクリック → `chat_unlocked` フラグ
**演出**: 5問全正解 → キツネ団長の反応変化 → Night3リンク出現
**フラグ**: `chat_q1_clear` 〜 `chat_q5_clear`, `chat_freemode`, `night3_unlocked`
